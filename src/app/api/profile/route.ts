    ...
    if (role === "SEARCHER" && body.searcherProfile) {
      ...
      await prisma.searcherProfile.upsert({
        ...
        update: {
          ...
          ...(sp.searchStatus !== undefined && { searchStatus: sp.searchStatus as "PREPARING" | "RAISING_CAPITAL" | "ACTIVELY_SEARCHING" | "LOI_SIGNED" | "ACQUIRED" | "OPERATING" | "EXITED" }),
          ...
          ...(sp.targetRevenueMin !== undefined && { targetRevenueMin: sp.targetRevenueMin as number | null }),
          ...(sp.targetRevenueMax !== undefined && { targetRevenueMax: sp.targetRevenueMax as number | null }),
          ...(sp.acquisitionTimeline !== undefined && { acquisitionTimeline: (sp.acquisitionTimeline as string) || null }),
          ...(sp.seekingCapital !== undefined && { seekingCapital: sp.seekingCapital as boolean }),
          ...(sp.capitalNeeded !== undefined && { capitalNeeded: sp.capitalNeeded as number | null }),
    ...
    if (role === "INVESTOR" && body.investorProfile) {
      ...
      await prisma.investorProfile.upsert({
        ...
        update: {
          ...
          ...(ip.totalDealsInvested !== undefined && { totalDealsInvested: ip.totalDealsInvested as number | null }),
          ...(ip.preferredSearchTypes !== undefined && { preferredSearchTypes: ip.preferredSearchTypes as ("TRADITIONAL" | "SELF_FUNDED" | "ACCELERATOR" | "INDEPENDENT_SPONSOR")[] }),
          ...(ip.preferredEbitdaMin !== undefined && { preferredEbitdaMin: ip.preferredEbitdaMin as number | null }),
          ...(ip.preferredEbitdaMax !== undefined && { preferredEbitdaMax: ip.preferredEbitdaMax as number | null }),
        },
        create: {
          userId: session.id,
