...
export default function DCFValuationSmallBusinessArticle() {
  return (
    <article>
      ...
      <p className={bodyClass}>
        DCF valuation is most powerful when the target business has characteristics that don&rsquo;t fit neatly into multiples-based frameworks. According to McKinsey&rsquo;s valuation research, DCF outperforms multiples when cash flows are predictable (recurring revenue, long-term contracts), when growth rates differ significantly from industry peers, when the capital structure will change post-acquisition, or when exit timing is uncertain. Aswath Damodaran&rsquo;s investment valuation framework recommends using DCF for any business where you have 3-5 years of reliable financial history and can build defensible revenue and margin projections. In practice, most sophisticated searchers use both methods: multiples for quick screening and market context, DCF for deep analysis of serious candidates, the two approaches should converge, and if they don&rsquo;t, the divergence itself reveals something important about the business.
      </p>

      <h3 className={h3Class}>What discount rate (WACC) is appropriate for small business acquisitions?</h3>

      <p className={bodyClass}>
        For most search fund acquisitions with 60-70% equity financing, expect WACC in the 12-18% range. Duff &amp; Phelps (now Kroll) and Ibbotson&rsquo;s research on size premiums shows that small businesses require a 3-6% additional premium over large-cap cost of equity due to customer concentration, limited management depth, and restricted access to capital. Shannon Pratt&rsquo;s valuation framework for small businesses recommends adding a further 0-5% company-specific risk premium for factors like customer concentration above 25%, single product lines, regulatory exposure, or management transition risk. The cost of equity for a typical SME acquisition therefore ranges from 14-22%, which when blended with after-tax debt costs of 7-8% (SBA 7(a) rates at Prime + 2.75%, tax-adjusted), produces the 12-18% WACC range common in search fund models.
      </p>

      <h3 className={h3Class}>Why does terminal value often represent 60-80% of enterprise value, and how do I avoid over-relying on it?</h3>
