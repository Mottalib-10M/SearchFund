...
async function getSearcher(slug: string) {
  try {
    ...
    if (searcher && !searcher.isPublic) return null;
    return searcher;
  } catch {
    return null;
  }
...
export default async function SearcherProfilePage({ params }: PageProps) {
  ...
  const personJsonLd = user.name ? {
    ...
    "@type": "Person",
    name: user.name,
    jobTitle: "Search Fund Entrepreneur",
    ...(user.linkedinUrl ? { sameAs: [user.linkedinUrl] } : {}),
    ...(country ? { nationality: country.name } : {}),
    worksFor: { "@type": "Organization", name: "SearchFundMarket", url: "https://www.searchfundmarket.com" },
  } : null;
