...
import {
  ...
  DEAL_COMPARISON,
  SEARCH_STRUCTURE,
  SEARCHER_DEMOGRAPHICS,
  INTL_SECTOR_DISTRIBUTION,
  INTL_SEARCHER_BACKGROUNDS,
  EXIT_PERFORMANCE,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title:
      "Search Fund Statistics 2024: 681 Funds, 35% IRR, 4.5x MOIC",
    description:
      "681 funds. 35.1% IRR. 4.5x MOIC. Verified data from Stanford GSB, IESE, and Yale SOM — returns, deal sizes, sectors, and failure rates all in one page.",
    openGraph: {
      title: "Search Fund Statistics 2024: 681 Funds, 35% IRR, 4.5x MOIC",
      description:
        "681 funds. 35.1% IRR. 4.5x MOIC. Verified data from Stanford GSB, IESE, and Yale SOM — returns, deal sizes, sectors, and failure rates all in one page.",
      type: "website",
    },
        ...
        <section className="mt-20">
          ...
          <div className="mt-4 grid gap-10 lg:grid-cols-2">
            <div>
              ...
              <div className="mt-4 space-y-2">
                {INTL_SECTOR_DISTRIBUTION.map((s) => (
                  <HorizontalBar
                    key={s.sector}
                    label={s.sector}
                    value={s.pct}
                    maxValue={Math.max(...INTL_SECTOR_DISTRIBUTION.map((x) => x.pct))}
                    color="#8b5cf6"
                  />
                ))}
              </div>
            ...
            <div>
              ...
              <div className="mt-4 space-y-2">
                {INTL_SEARCHER_BACKGROUNDS.map((b) => (
                  <HorizontalBar
                    key={b.field}
                    label={b.field}
                    value={b.pct}
                    maxValue={Math.max(...INTL_SEARCHER_BACKGROUNDS.map((x) => x.pct))}
                    color="#a855f7"
                  />
                ))}
              </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
            {EXIT_PERFORMANCE.map((e) => (
              ...
              >
                ...
                <div className="mt-2">
                  <SourceBadge sourceId={e.sourceId} />
                </div>
              </div>
            ))}
          </div>
